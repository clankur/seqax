...
# Defaults for LCM runs that didn't override these fields (from configs/slim_v4-8_31m.yaml,
# which inherits from configs/slim_base.yaml).
LCM_CONFIG_DEFAULTS = {
    "d_model": 256,
    "d_ff": 1024,
    "n_kv": 16,
...
def get_eval_metrics_table(metrics_data, config_data, top_k=None):
    ...
    for guid, d in metrics_data.items():
        ...
        for config_key, display_key in TRACKED_FIELDS:
            ...
            if val is None and d["kind"] in ("lcm", "leaky"):
                val = LCM_CONFIG_DEFAULTS.get(config_key)
            if val is None:
                continue
