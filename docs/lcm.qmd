...
def get_metrics_data(experiments_dir):
    ...
    for exp_dir in sorted(experiments_dir.iterdir()):
        ...
        if loss_csv is None:
            continue
        steps, loss = loss_csv
        final_loss = _read_csv(exp_dir / "metrics" / "final_loss_eval.csv")
        final_perplexity = _read_csv(exp_dir / "metrics" / "final_perplexity_eval.csv")
        ...
        metrics_data[task_id] = {
            ...
            "final_perplexity": final_perplexity[1] if final_perplexity else None,
        }
...
metrics_data, config_data = get_metrics_data(LCM_EXPERIMENTS_DIR)
```

## concept_size=1 control

A 31M parameter model was trained on SlimPajama at sequence length 2048. The [baseline](../configs/slim_v4-32_31m.yaml) without any concept architecture outperforms every LCM variant across the full training run.
...
```{python}
plot_loss_data(metrics_data, ema_smoothing=0.97, top_k=10, opacity=0.05)
plot_loss_data(metrics_data, ema_smoothing=0.99, top_k=5)
```
...
```{python}
get_eval_metrics_table(metrics_data, config_data, top_k=10)
```
...
31M parameters at $L$ = 2048 is below the regime where LCM-style methods are expected to help. The LCM paper's claims center on long-context semantic compression. A negative result at this scale does not rule out the architecture at larger scales or longer contexts.
...
The encoder produces concept $i$ from block $i$. The token decoder consumes concept $i$ to predict tokens in block $i$. With `>=`, tokens in block $i$ attend to themselves through the concept embedding, introducing future leakage. Training loss improves. Autoregressive quality degrades. Accepting either fix without tracing the data flow introduces a silent correctness bug.