...
Opus 4.7 on high reviewed this code and flagged two "bugs":
...
Both fixes change `>` to `>=`. Concept $i$ is produced from block $i$ and consumed by the token decoder to predict tokens in block $i$. With `>=`, tokens in block $i$ attend to themselves through the concept embedding. This is future leakage.

The `>=` mask shipped accidentally for an 8-hour window on 2025-01-04 → 2025-01-05. Runs from that window form a leaky cohort and are excluded from the analyses above. The plot below shows leaky runs that beat the baseline:
...
The LLM review recommends the bug. Training loss prefers it. Catching it requires tracing the data flow by hand.